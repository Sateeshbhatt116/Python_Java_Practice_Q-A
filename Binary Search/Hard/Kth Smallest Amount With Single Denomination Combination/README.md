...
For a given value mid, count how many distinct amounts <= mid can be formed using at least one coin denomination.

Since coins of different denominations cannot be combined, an amount is valid if it is divisible by at least one coin.

Use the Inclusion-Exclusion Principle to avoid counting common multiples multiple times.

If count(mid) >= k, the kth smallest amount is <= mid, so search on the left side.

Otherwise, search on the right side.

Finally, the left value will be the kth smallest amount.

Time Complexity: O(n × 2^n × log(answer))
